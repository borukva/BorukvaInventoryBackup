...
public class EnderChestGui extends SimpleGui {
    ...
    private void addItems(Map<Integer, ItemStack> enderChestMap, String playerName,SimpleGui caller){
        ...
        this.setSlot(35, new GuiElementBuilder(Items.PAPER)
                ...
                .setCallback((index, type, action) -> {
                    UUID uuid = InventoryGui.getOfflinePlayerProfile(playerName, player.getEntityWorld().getServer());

                    if(this.player.getEntityWorld().getServer().getPlayerManager().getPlayer(playerName) != null){
                        backUpPlayerItems(enderChestMap, this.player.getEntityWorld().getServer().getPlayerManager().getPlayer(playerName));
